...
export class EnvironmentService {
  ...
  async create(createEnvironmentDto: CreateEnvironmentDto) {
      ...
      this.httpService.get(this.getUserEndpoint + createEnvironmentDto.requestUserId).pipe(
        catchError((error) => {
          if (error.response.status === 'ECONNREFUSED') {
            ...
            this.logCerberusService.create(
              LogsCerberusConstants.createEnvironmentVerifyRolesFailedById({
            ...
            this.logCerberusService.create(
              LogsCerberusConstants.createEnvironmentVerifyRolesFailedByUser({
                userId: createEnvironmentDto.requestUserId,
                error: error.response.data.message,
                statusCode: 404
              })
    ...
    if (!user || user?.user_role?.lenght > 1 || user?.user_role[0]?.role_id !== 1) {
      this.logCerberusService.create(
        LogsCerberusConstants.createEnvironmentVerifyRolesFailedByRole({
          userId: createEnvironmentDto.requestUserId,
    ...
    try {
      ...
      if (error.code === 'P2002') {
        this.logCerberusService.create(
          LogsCerberusConstants.verifyRolesFailedByEnvironment({
  ...
  async sendLogWhenEnvironmentCreated(environmentName: string, environmentId: string, userId: string, createEnvironmentDto: CreateEnvironmentDto) {
    ...
    await this.logCerberusService.create(
      LogsCerberusConstants.createEnvironmentSuccess(
        user.name,
        environmentName,
        {
          environmentId,
  ...
  async createTemporaryAccess(createTemporaryAccessDto: CreateTemporaryAccessDto) {
    const isAdmin = await lastValueFrom(
      this.httpService.post(this.verifyRoleEndpoint, {
  ...
  async sendLogWhenTemporaryAccessCreated(
    ...
    await this.logCerberusService.create(
      LogsCerberusConstants.createTemporaryAccessSuccess(
        userName,
        environment.name,
        createdByUser.name,
        {
  ...
  async findAll(findAllDto: FindAllDto) {
    ...
    try {
      ...
      this.logCerberusService.create(LogsCerberusConstants.findAllError({ target: 'users', statusCode: 500 }))
  ...
  async sendLogWhenEnvironmentUpdated(environmentName: string, environmentId: string, userId: string, updateEnvironmentDto: UpdateEnvironmentDto) {
    ...
    await this.logCerberusService.create(
      LogsCerberusConstants.updateEnvironmentSuccess(
        user.name,
        environmentName,
        {
          environmentId,
  ...
  async sendLogWhenEnvironmentStatusChanged(environmentName: string, environmentId: string, userId: string, status: boolean) {
    ...
    await this.logCerberusService.create(
      LogsCerberusConstants.changeEnvironmentStatusSuccess(
        user.name,
        environmentName,
        status,
        {
  ...
  async sendLogWhenEnvironmentRemoved(environmentName: string, environmentId: string, userId: string) {
    ...
    await this.logCerberusService.create(
      LogsCerberusConstants.removeEnvironmentSuccess(
        user.name,
        environmentName,
        {
          environmentId,
