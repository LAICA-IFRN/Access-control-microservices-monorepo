...
async function bootstrap() {
  ...
  app.setGlobalPrefix('application/access-control')
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('Documentação da aplicação de controle de acesso do Laica Cerberus')
    .setVersion('0.1')
    .build();

  const document = SwaggerModule.createDocument(app, config);
  SwaggerModule.setup('access-control/api', app, document);
