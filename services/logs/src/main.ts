...
async function bootstrap() {
  ...
  app.setGlobalPrefix('service/logs');

  const config = new DocumentBuilder()
    .setTitle('Serviço de Logs')
    .setDescription('Documentação do serviço de logs do Laica Cerberus')
    .setVersion('0.1')
    .build();

  const document = SwaggerModule.createDocument(app, config);
  SwaggerModule.setup('service/logs/api', app, document);
